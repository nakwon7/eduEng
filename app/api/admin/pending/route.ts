...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin
    .from("profiles")
    .select("id, username, name, email, level, trial_calls, expires_at, unlimited, blocked, total_seconds, created_at, ko_access, payment_requested_at, payment_note, plan, requested_plan")
